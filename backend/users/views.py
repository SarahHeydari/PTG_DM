...
from .models import User, AccessGroup, GroupMember, Report
from .serializers import (
    RegisterSerializer, LoginSerializer, ChangePasswordSerializer,
    AccessGroupCreateSerializer, AccessGroupListSerializer, AccessGroupUpdateSerializer,
    GroupMemberAddSerializer, GroupMemberListSerializer,
    UserListSerializer, UserMeSerializer, AdminUserListSerializer,
...
class GroupMemberRemoveAPIView(APIView):
    ...
    def delete(self, request, group_id: int, user_id: int):
        ...
        if not membership:
            return Response({"detail": "این کاربر عضو این گروه نیست یا قبلاً حذف شده است."}, status=status.HTTP_404_NOT_FOUND)

        group_name = membership.group.name
        username = membership.user.username
        membership.delete()

        return Response({"detail": f"کاربر {username} از گروه {group_name} حذف شد."}, status=status.HTTP_200_OK)
...
class GroupMembersListAPIView(APIView):
    ...
    def get(self, request, group_id: int):
        ...
        if request.user.role not in ["manager", "admin"]:
            ...
            if not is_member:
                return Response({"detail": "شما دسترسی مشاهده اعضای این گروه را ندارید."}, status=status.HTTP_403_FORBIDDEN)
        ...
        return Response(
            {"group": {"id": group.id, "name": group.name, "access_level": group.access_level}, "members": data},
            status=status.HTTP_200_OK,
        )
...
class AccessGroupUpdateAPIView(APIView):
    ...
    def patch(self, request, group_id: int):
        ...
        return Response({"detail": "گروه با موفقیت به‌روزرسانی شد.", "group": serializer.data}, status=status.HTTP_200_OK)
...
class UserMeAPIView(APIView):
    ...
    def patch(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response({"detail": "پروفایل با موفقیت به‌روزرسانی شد.", "user": serializer.data}, status=status.HTTP_200_OK)
...
class AdminPingAPIView(APIView):
    ...
    def get(self, request):
        return Response(
            {"ok": True, "user": {"id": request.user.id, "username": request.user.username, "role": request.user.role, "email": request.user.email}},
            status=status.HTTP_200_OK,
        )
...
class AdminUserCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if role not in ["admin", "manager", "expert"]:
            return Response({"detail": "role must be one of: admin, manager, expert."}, status=status.HTTP_400_BAD_REQUEST)

        if User.objects.filter(username=username).exists():
            return Response({"detail": "این نام کاربری قبلاً ثبت شده است."}, status=status.HTTP_400_BAD_REQUEST)

        user = User.objects.create(username=username, password=password, role=role, email=email)

        return Response({"detail": "کاربر با موفقیت ایجاد شد.", "user": {"id": user.id, "username": user.username, "role": user.role, "email": user.email}}, status=status.HTTP_201_CREATED)
...
class AdminUserDeleteAPIView(APIView):
    ...
    def delete(self, request, user_id: int):
        if request.user.id == user_id:
            return Response({"detail": "ادمین نمی‌تواند خودش را حذف کند."}, status=status.HTTP_400_BAD_REQUEST)

        target = User.objects.filter(id=user_id).first()
        if not target:
            return Response({"detail": "کاربر موردنظر یافت نشد."}, status=status.HTTP_404_NOT_FOUND)

        if target.role == "admin" and User.objects.filter(role="admin").count() <= 1:
            return Response({"detail": "امکان حذف آخرین ادمین وجود ندارد."}, status=status.HTTP_400_BAD_REQUEST)

        username = target.username
        target.delete()
        return Response({"detail": f"کاربر {username} حذف شد."}, status=status.HTTP_200_OK)
...
class AdminStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        top_groups_data = [{"id": g.id, "name": g.name, "members_count": g.members_count} for g in top_groups]
...
class ReportUploadAPIView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"detail": "گزارش با موفقیت آپلود شد.", "report": {"id": report.id, "title": report.title, "subsystem": report.subsystem, "file_url": file_url, "created_at": report.created_at}},
            status=status.HTTP_201_CREATED,
        )
...
class MyReportsAPIView(APIView):
    ...
    def get(self, request):
        ...
        if q:
            qs = qs.filter(Q(title__icontains=q) | Q(subsystem__icontains=q))
